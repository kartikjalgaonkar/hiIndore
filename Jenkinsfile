     ...
     // sh "kubectl config use-context minikube"
      ...
      //   sh "minikube start --vm-driver=virtualbox"
        ...
        //           sh("sed -i.bak 's#hub.docker.com/${project}/${appName}:${imageVersion}#${imageTag}#' ./k8s/development/*.yaml")
         ...
         sh("echo http://`kubectl --namespace=${namespace} get ${feSvcName} --output=json --kubeconfig=/home/yash/.kube/config | jq -r '.status.loadBalancer.ingress[0].ip'` > ${feSvcName} --kubeconfig=/home/yash/.kube/config")
                   break
